...
// ---------------------------------------------------------------------------
// Koreksi typo ringan untuk istilah wisata penting (vila, cafe, resto, dst).
// Model lokal terbukti bisa salah paham typo (mis. "fila" dikira "film"
// alih-alih "vila"), jadi kata kunci penting dikoreksi di KODE dulu sebelum
// dikirim ke AI — jangan cuma mengandalkan AI menebak sendiri.
// ---------------------------------------------------------------------------
...
// Istilah wisata NICHE yang beneran berisiko disalahartikan kalau typo (misal
// "fila" bisa kebaca "film" oleh model). SENGAJA tidak memasukkan kata umum
// bahasa Indonesia sehari-hari (mis. "sana", "bagus", "harga") karena kata-kata
// itu sudah dipahami model dengan baik apa adanya, dan malah rawan salah
// koreksi kalau dimasukkan (mis. "saja" bisa ketiban dikoreksi jadi "sana").
// Tambahkan istilah baru di sini HANYA kalau memang niche & terbukti bikin
// AI salah paham saat typo.
const DOMAIN_KEYWORDS = [
  "vila", "villa", "cafe", "kafe", "resto", "restoran", "hotel", "penginapan",
  "dermaga", "snorkeling",
];

/**
 * Koreksi ringan typo pada kata-kata kunci wisata niche di teks pertanyaan
 * pengguna, berdasarkan jarak-edit (Levenshtein) ke daftar istilah baku di
 * atas. Sengaja SANGAT ketat (harus beda PERSIS 1 huruf, panjang kata mirip)
 * supaya tidak salah mengoreksi kata biasa yang kebetulan mirip.
 */
function correctTypos(text) {
  if (!text) return text;
  return text.replace(/[A-Za-zÀ-ÿ]+/g, (word) => {
    const lower = word.toLowerCase();
    if (lower.length < 3 || DOMAIN_KEYWORDS.includes(lower)) return word;

    let best = null;
    let bestDist = Infinity;
    for (const kw of DOMAIN_KEYWORDS) {
      if (Math.abs(kw.length - lower.length) > 1) continue;
    ...
    return best && bestDist === 1 ? best : word;
  });
}
...
// Kata-kata geografis umum yang SERING jadi bagian nama destinasi tapi TIDAK
// khas/unik (banyak destinasi berbeda bisa punya kata ini) — jangan dipakai
// sebagai satu-satunya dasar pencocokan, supaya "pantai" doang gak nyasar ke
// pantai yang salah.
const GENERIC_PLACE_WORDS = new Set([
  "tanjung", "pantai", "pulau", "gunung", "air", "terjun", "danau", "bukit",
  "puncak", "gua", "tebing", "taman", "hutan", "desa", "kampung", "situs",
  "benteng", "pemandian", "wisata", "kota", "kabupaten", "kawasan", "spot",
]);

/**
 * Deteksi apakah pengguna menyebut sebuah destinasi. Dua tahap:
 * 1) Match nama LENGKAP persis (paling akurat, dicoba dulu).
 * 2) Fallback: match kata KHAS dari nama destinasi (bukan kata geografis umum
 *    seperti "tanjung"/"pantai") sebagai whole-word match di teks pertanyaan.
 *    Ini menangani kasus user cuma nyebut sebagian nama, mis. "Bira" saja
 *    tanpa "Tanjung" — sebelumnya kasus ini GAGAL match dan malah jatuh ke
 *    text-search bebas yang bisa nyasar ke destinasi lain sama sekali
 *    (misalnya ke "Pulau Kapoposang").
 */
function detectExplicitDestination(text, destinations) {
  ...
  const wordsInText = new Set(lower.match(/[a-zà-ÿ]+/g) || []);
  let bestMatch = null;
  let bestSpecificity = 0;
  for (const d of destinations) {
    const nameWords = (d.name.toLowerCase().match(/[a-zà-ÿ]+/g) || []).filter(
      (w) => w.length >= 4 && !GENERIC_PLACE_WORDS.has(w)
    );
    for (const w of nameWords) {
      if (wordsInText.has(w) && w.length > bestSpecificity) {
        bestSpecificity = w.length;
        bestMatch = d;
      }
    }
  }
 ...
 */
async function askAI({ question: rawQuestion, userLocation, destinationName, regency, conversationHistory }) {
  const catalog = await getCatalog();

  // Koreksi typo istilah wisata penting (mis. "fila" -> "vila") sebelum dipakai
  // untuk deteksi destinasi/kabupaten maupun dikirim ke model.
  const question = correctTypos(rawQuestion);
  ...
  // Deteksi apakah pertanyaan ini secara EKSPLISIT menyebut nama kabupaten
  // (bukan cuma warisan dari histori) TANPA menyebut destinasi spesifik apapun
  // — kalau iya, ini pertanyaan UMUM soal kabupaten tsb (mis. "apa yang bagus
  // di Bulukumba?"), BUKAN lanjutan pertanyaan soal destinasi spesifik yang
  // dibahas sebelumnya. Dipakai supaya AI gak "nyangkut" ke destinasi lama
  // dari histori padahal user jelas-jelas nanya rekomendasi umum sekabupaten.
  const regencyMentionedThisTurn = detectExplicitRegency(question, catalog.regencies);
  const destinationMentionedThisTurn = detectExplicitDestination(question, catalog.destinations);
  const isGeneralRegencyQuestion = !!regencyMentionedThisTurn && !destinationMentionedThisTurn;
  ...
  if (!mainDestination && !isGeneralRegencyQuestion) {
...
JADILAH ASISTEN YANG "NYAMBUNG" (BUKAN CUMA JAWAB HARFIAH):
- Pahami MAKSUD pertanyaan, bukan cuma kata-katanya secara harfiah. Kalau user tanya "di mana", dia
  mau tau LOKASI, bukan cuma jarak tempuh. Kalau dia tanya lanjutan singkat, kaitkan dengan destinasi
  yang sedang dibahas di riwayat percakapan — jangan minta dia mengulang nama tempatnya lagi.
- Baca riwayat percakapan sebelum jawab, supaya jawabanmu terasa nyambung sama obrolan sebelumnya
  (misal gak perlu jelasin ulang hal yang sudah kamu sebut di jawaban sebelumnya, kecuali diminta).
- Kalau ada info relevan yang nyambung secara natural dengan yang ditanya (dan memang ada di data),
  boleh disebut singkat sebagai pelengkap — tapi tetap dalam batas "jawab sesuai yang ditanya", jangan
  jadi alasan buat dump semua data lagi.
...
ATURAN TYPO/SALAH KETIK:
- Pertanyaan pengguna sering ada typo atau singkatan gak baku (mis. "sna" = "sana", "bgus" = "bagus",
  "fila"/"vla" = "vila"). Pahami maksudnya dari KONTEKS WISATA (nama tempat, vila, cafe/resto, spot
  foto, jam buka, harga, dst) — jangan artikan typo jadi kata lain yang gak nyambung sama tema wisata
  (misal jangan artikan "fila" sebagai "film").
- Kalau memang gak yakin maksud pertanyaannya apa walau sudah dicoba dipahami dari konteks, boleh
  tanya balik dengan santai untuk klarifikasi, daripada menjawab ngasal.
...
- Kalau ada bagian yang ditanya (misal vila, cafe, spot foto) TIDAK ADA datanya untuk destinasi ini,
  bilang jujur dan singkat bahwa datanya belum ada, lalu berhenti di situ. DILARANG KERAS menebak atau
  merekomendasikan nama tempat/kota lain yang TIDAK ADA di data konteks (mis. jangan bilang "coba cek
  di kota X" kalau kamu sendiri gak punya data soal kota X itu) — itu sama saja mengarang informasi.
  Cukup sarankan verifikasi langsung ke pengelola destinasi ini.

ATURAN KHUSUS PERTANYAAN LOKASI ("di mana", "lokasinya di mana", "letaknya di mana"):
- Kalau pengguna nanya di mana lokasi/letak suatu destinasi, jawaban WAJIB menyebutkan kabupaten/kota
  tempat destinasi itu berada (field "regency" di data konteks) sebagai bagian utama jawaban — bukan
  cuma cerita estimasi jarak/waktu tempuh dari lokasi pengguna. Estimasi jarak boleh ditambahkan
  sebagai info pelengkap SETELAH menyebut lokasinya, kalau memang datanya ada.
- Kalau ada catatan akses jalan di data konteks, boleh disebut singkat untuk menambah konteks lokasi.
